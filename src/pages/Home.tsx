  ...
  const showDB = () => {
    try {
      SQLite.create({
        ...
        try {
          const select = await db.executeSql("select * from danceMoves");
          await console.log("Table created/exists. Msg: ", select);
          setData(select);
        } catch (e) {
          console.log("SQL error: ", e);
        }
    ...
    <IonPage>
      ...
      <IonContent>
        ...
        <IonAlert
          ...
          buttons={["OK"]}
        />
        <IonButton onClick={showDB}>Show DB</IonButton>
        <IonAlert
          isOpen={showData}
          onDidDismiss={() => setShowData(false)}
          header={"Select!"}
          message={data}
          buttons={["OK"]}
        />
